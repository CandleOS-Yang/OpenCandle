...
int main(int argc, char *argv[])
{
    ...
    else if (argc == 2)
    {
        if (argv[1][0] == '-' && argv[1][1] == 'h' && argv[1][2] == '\0')
        {
            ...
            printf("fvdisk.exe [.bin] -o [.vhd/.img] -s [start sector num]\n\n");
            printf("########## *GENERAL* Make Image File ##########\n");
            printf("fvdisk.exe -i [.vhd/.img] -n [sector num]\n\n");
            printf("########## *FAT* Write File ##########\n");
            printf("fvdisk.exe [.bin] -o [.vhd/.img] -f [partition num]\n\n");
            printf("########## *FAT* View Partition Info ##########\n");
            printf("fvdisk.exe -i [.vhd/.img] -v [partition num]\n\n");
            printf("########## *FAT* View DBR Sector Info ##########\n");
            printf("fvdisk.exe -i [.vhd/.img] -v DBR [partition num]\n\n");
        }
        else
        {
            printf("fvdisk.exe -h\n");
    ...
    {
        //*普通*写入文件
        //fvdisk.exe [.bin] -o [.vhd/.img] -s [start sector num]
        ...
        //*FAT文件系统*写入文件
        //fvdisk.exe [.bin] -o [.vhd/.img] -f [partition num]
        ...
        //*FAT文件系统*查看DBR扇区信息
        //fvdisk.exe -i [.vhd/.img] -v DBR [partition num]
    ...
    {
        //*普通*创建镜像文件
        //fvdisk.exe -i [.vhd/.img] -n [sector num]
        ...
        //*FAT文件系统*查看分区信息
        //fvdisk.exe -i [.vhd/.img] -v [partition num]
...
int view_partition_info(FILE *infile_p, char *argv[], struct FAT_BPB fat_bpb, struct FAT_START_ADDR *fat_start_addr)
{
    ...
    unsigned int fat1_start_addr = dbr_start_addr + dbr_size + reserved_size;
